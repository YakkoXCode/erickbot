...
Bonus: ${poin} XP
    `.trim()
  conn.tebakgambar[id] = [
    await conn.sendButtonImg(m.chat, caption, await (await fetch(json.img)).buffer(), '© Erick', 'BANTUAN', '.hint')
    ,
    json, poin,
    setTimeout(async () => {
      if (conn.tebakgambar[id]) await conn.sendButton(m.chat, `Waktu habis!\nJawabannya adalah *${json.jawaban}*`, '© Erick', 'TEBAK GAMBAR', '.tebakgambar')
